...
public class Java8Streams {
	public static void main(String args[]) { // Count the empty strings
		...
		// Count number of String which startswith "a" 
		 ...
		 System.out.printf("Original List : %s, List without Empty Strings : %s %n", strList, filtered); // Create a
		// List with String more than 2	 characters 
		 filtered = strList.stream() .filter(x -> x.length()> 2).collect(Collectors.toList());
		 System.out.printf("Original List :		 %s, filtered list : %s %n", strList,		 filtered); 
		 // Convert String to
		// Uppercase and join them using coma
		...
		//Get count,
		// min, max, sum, and average for numbers 
